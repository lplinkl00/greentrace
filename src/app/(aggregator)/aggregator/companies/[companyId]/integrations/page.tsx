...
export default function AggregatorCompanyIntegrationsPage({ params }: { params: { companyId: string } }) {
    ...
    const fetchConfigs = async () => {
        try {
            const res = await fetch(`/api/integration-configs?companyId=${params.companyId}`)
    ...
    return (
        <div className="space-y-6 max-w-4xl">
            <div>
                <h1 className="text-2xl font-bold text-gray-900">Company Integrations</h1>
                <p className="text-sm text-gray-500 mt-1">Manage system connections for Company ID: {params.companyId}</p>
            </div>
