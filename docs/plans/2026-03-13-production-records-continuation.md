# Production Records — Continuation Plan

> **For Claude:** Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Complete the production records feature — all schema/DB work is done, finish the data layer, API routes, pages, sidebar, and close the Notion ticket.

**Architecture:** New `ProductionRecord` Prisma model + migration already applied. Need: `src/lib/production.ts`, `/api/production` route, two pages under `(mill)/mill/production/`, sidebar update, and Notion ticket close.
...
- ✅ `prisma/schema.prisma` — `ProductionSource` enum and `ProductionRecord` model added (committed)
- ⚠️ `prisma/migrations/20260313125704_add_production_record/` — migration applied to DB but **not yet committed to git**
- ❌ All remaining tasks below

---

### Task 1: Commit the pending migration

**Files:**
- `prisma/migrations/20260313125704_add_production_record/migration.sql` (already exists, not committed)

**Step 1: Stage and commit**

```bash
cd "D:/Claude Code"
git status  # confirm migration file is untracked
...
### Task 2: Create `src/lib/production.ts` data access layer
...
### Task 3: Add API routes for production records
...
Follow the same pattern as `src/app/api/imports/route.ts`. The `withAuth` signature is: `withAuth(roles[], async (request, context, user) => {...})` — 3 params.
...
### Task 4: Create the `/mill/production` list page

**Files:**
- Create: `src/app/(mill)/mill/production/page.tsx`

Note: The route group `(mill)` does NOT add a URL segment — the URL is `/mill/production`.
...
### Task 5: Create the `/mill/production/new` form page
...
### Task 6: Add "Production" nav item to the sidebar

**Files:**
- Modify: `src/components/AppSidebar.tsx`

**Step 1: Add `Factory` to the lucide-react import**
...
**Step 2: Add Production nav item to the mill section**
...
### Task 7: Update the Notion ticket status to Done

Update ticket #3 "Change Imports to Production for Mills" in the Notion ticketing database.

- Page ID: `3227ee21-af0e-80e1-abec-c3f4f6e93c0a`
- Change `Status` from `Backlog` → `Done`

Use the `notion-update-page` tool:
```
page_id: 3227ee21-af0e-80e1-abec-c3f4f6e93c0a
